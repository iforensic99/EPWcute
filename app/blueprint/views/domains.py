...
# @admin.route("/domian_get", methods=["GET"])
# @admin_required
# def domains_get():
#     if request.method == "POST":
#         domain_name = request.form.get('domain', None)
#         table_name = domain_name.replace('.', '_')
#         table_name = table_name + '_resolve_result'
#         conn = sqlite3.connect(r'result.sqlite3')
#         cursor = conn.cursor()
#         target_list = []
#         results = cursor.execute(f'select subdomain from "{table_name}" ')
